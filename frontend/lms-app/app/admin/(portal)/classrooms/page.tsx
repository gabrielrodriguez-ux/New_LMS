...
export default function AdminClassroomsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Map keys if needed (already aligned in backend refactor)
            ...
            const p = Array.from(new Set(coursesData.map((c: any) => c.promotion).filter(Boolean))) as string[];
            setPromotions(p);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert('Error al guardar el aula. Verifica la conexión con el servidor.');
            console.error(error);
        }
    };
